...
// All field types except plain text can be shown as card badges
export const CARD_ELIGIBLE_TYPES = ['select', 'multi_select', 'textarea', 'number', 'date', 'checkbox', 'rating', 'url'];
...
export function FormBuilder({ initialConfig, initialCustomFields, onConfigChange }) {
  ...
  // Delete a custom field — removes from local array
  const handleDeleteField = (field) => {
    if (!field.isCustom) return;

    // Remove field from the local array entirely
    const updated = customFields.filter((f) => f.id !== field.id);
    setCustomFields(updated);

    // Also remove from card_fields and analytics_fields config if present
    const fieldKey = `custom_${field.id}`;
    let newConfig = { ...config };
    if (Array.isArray(newConfig.card_fields)) {
      newConfig.card_fields = newConfig.card_fields.filter((k) => k !== fieldKey);
    }
    if (Array.isArray(newConfig.analytics_fields)) {
      newConfig.analytics_fields = newConfig.analytics_fields.filter((k) => k !== fieldKey);
    }
    if (Array.isArray(newConfig.field_order)) {
      newConfig.field_order = newConfig.field_order.filter((k) => k !== fieldKey);
    }
    setConfig(newConfig);
    notifyChange(newConfig, updated);
    ...
    if (!isOnboarding) {
      ...
      pendingModifiedFieldIds.current.delete(field.id);
    }
  };
  ...
  const handleCardIneligible = useCallback(() => {
    toast.error('Text fields cannot be shown as card badges.');
  }, [toast]);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="lg:col-span-2">
        <div className="lg:sticky lg:top-20 max-h-[calc(100vh-6rem)] overflow-y-auto scrollbar-on-hover space-y-4">
          {/* Preview header + tab switcher */}
          <div className="bg-white dark:bg-[#0D1117] border border-neutral-200 dark:border-[#30363D]/60 rounded-xl p-3">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-semibold text-neutral-900 dark:text-[#E6EDF3]">Live Preview</span>
              </div>
              <span className="text-[10px] text-neutral-400 dark:text-[#484F58]">Updates in real-time</span>
            </div>
            <div className="flex gap-1 p-1 bg-neutral-50 dark:bg-[#161B22] rounded-lg">
              <button
                onClick={() => setPreviewTab('form')}
                className={`flex-1 py-1.5 text-xs font-medium rounded-md transition-colors duration-100 ${
                  previewTab === 'form'
                    ? 'bg-white dark:bg-[#21262D] text-neutral-900 dark:text-[#E6EDF3] shadow-sm'
